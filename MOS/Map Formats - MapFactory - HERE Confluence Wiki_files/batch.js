...
define("confluence/effects",["document","window"],function(b,g){return{placeFocus:function(){if(!b.location.hash&&!b.getElementById("editpageform")&&!b.getElementById("createpageform")&&!b.getElementById("upload-attachments")&&!b.getElementById("addcomment")){var a="",f=g.location.search.substring(1).split("&"),d;for(d=0;d<f.length;d++){var e=f[d].split("=")[0],c=f[d].split("=")[1];"autofocus"==e&&(null!=c&&0<c.length)&&(a="'"+c+"'")}f=!1;for(d=0;d<b.forms.length;d++){e=b.forms[d].elements;if("quick-search"!=
b.forms[d].id&&"space-blog-quick-search"!=b.forms[d].id&&"inlinecommentform"!=b.forms[d].name)for(c=0;c<e.length;c++)if(("text"==e[c].type||"password"==e[c].type||"textarea"==e[c].type)&&!e[c].disabled&&"none"!=e[c].style.display)try{if(null!=a&&0<a.length){if(e[c].id==a){e[c].focus();f=!0;break}}else{e[c].focus();f=!0;break}}catch(h){}if(f)break}}},setCookie:function(a,f,d,e,c,h,i,g){a=a+"="+escape(f);d&&(a+="; expires="+(new Date(d,e,c)).toGMTString());a=h?a+("; path="+escape(h)):a+"; path=/";i&&
(a+="; domain="+escape(i));g&&(a+="; secure");b.cookie=a},getCookie:function(a){return(a=b.cookie.match(a+"=(.*?)(;|$)"))?unescape(a[1]):null},highlight:function(a){new Effect.Highlight(a,{endcolor:"#f0f0f0"})}}});require("confluence/module-exporter").safeRequire("confluence/effects",function(b){window.placeFocus=b.placeFocus;window.setCookie=b.setCookie;window.getCookie=b.getCookie;window.highlight=b.highlight});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
